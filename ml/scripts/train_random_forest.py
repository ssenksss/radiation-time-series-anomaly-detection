...
def load_labeled_feature_measurements(dataset_id: int) -> pd.DataFrame:
    ...
    if len(np.unique(dataframe["original_label"])) < 2:
        raise RuntimeError("Random Forest requires both normal and anomaly examples.")
...
def train_random_forest(dataframe: pd.DataFrame, threshold: float) -> pd.DataFrame:
...
def replace_random_forest_results(dataset_id: int, results: pd.DataFrame) -> None:
    execute_query(
        """
...
def train_random_forest_for_active_dataset() -> int:
    dataset_id = get_active_dataset_id()
    threshold = get_threshold()

    dataframe = load_labeled_feature_measurements(dataset_id)
    results = train_random_forest(dataframe, threshold)
    replace_random_forest_results(dataset_id, results)

    print("Random Forest results written to anomaly_results.")
    return dataset_id


def main():
    train_random_forest_for_active_dataset()
