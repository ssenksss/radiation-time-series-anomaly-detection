...
def train_pca_reconstruction(dataframe: pd.DataFrame, threshold: float) -> pd.DataFrame:
    ...
    n_components = min(3, scaled_features.shape[1] - 1)
    ...
    anomaly_scores = np.mean((scaled_features - reconstructed) ** 2, axis=1)
...
def replace_pca_reconstruction_results(dataset_id: int, results: pd.DataFrame) -> None:
    execute_query(
        """
        DELETE FROM anomaly_results
        WHERE dataset_id = %s
          AND model_name = 'PCA Reconstruction Error';
        """,
        (dataset_id,),
    )
...
def train_pca_reconstruction_for_active_dataset() -> int:
    dataset_id = get_active_dataset_id()
    threshold = get_threshold()

    feature_dataframe = load_feature_measurements(dataset_id)
    result_dataframe = train_pca_reconstruction(feature_dataframe, threshold)
    replace_pca_reconstruction_results(dataset_id, result_dataframe)
    ...
    print("PCA Reconstruction Error training completed successfully.")
