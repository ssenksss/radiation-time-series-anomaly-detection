...
def load_labeled_feature_measurements(dataset_id: int) -> pd.DataFrame:
    ...
    if len(np.unique(dataframe["original_label"])) < 2:
        raise RuntimeError("Logistic Regression requires both normal and anomaly examples.")
...
def train_logistic_regression(dataframe: pd.DataFrame, threshold: float) -> pd.DataFrame:
    ...
    predictions = model.predict(scaled_features).astype(bool)
...
def replace_logistic_regression_results(dataset_id: int, results: pd.DataFrame) -> None:
    execute_query(
        """
...
def train_logistic_regression_for_active_dataset() -> int:
    dataset_id = get_active_dataset_id()
    threshold = get_threshold()

    dataframe = load_labeled_feature_measurements(dataset_id)
    results = train_logistic_regression(dataframe, threshold)
    replace_logistic_regression_results(dataset_id, results)

    print("Logistic Regression results written to anomaly_results.")
    return dataset_id


def main():
    train_logistic_regression_for_active_dataset()
