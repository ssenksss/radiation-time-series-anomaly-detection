...
def train_hbos(dataframe: pd.DataFrame, threshold: float) -> pd.DataFrame:
...
def replace_hbos_results(dataset_id: int, results: pd.DataFrame) -> None:
    execute_query(
        """
...
def train_hbos_for_active_dataset() -> int:
    dataset_id = get_active_dataset_id()
    threshold = get_threshold()

    feature_dataframe = load_feature_measurements(dataset_id)
    result_dataframe = train_hbos(feature_dataframe, threshold)
    replace_hbos_results(dataset_id, result_dataframe)
    ...
    print("HBOS training completed successfully.")
