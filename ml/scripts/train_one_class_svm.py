...
def train_one_class_svm(dataframe: pd.DataFrame, threshold: float) -> pd.DataFrame:
...
def replace_one_class_svm_results(dataset_id: int, results: pd.DataFrame) -> None:
    execute_query(
        """
        DELETE FROM anomaly_results
        WHERE dataset_id = %s
          AND model_name = 'One-Class SVM';
        """,
        (dataset_id,),
    )
...
def train_one_class_svm_for_active_dataset() -> int:
    dataset_id = get_active_dataset_id()
    threshold = get_threshold()

    feature_dataframe = load_feature_measurements(dataset_id)
    result_dataframe = train_one_class_svm(feature_dataframe, threshold)
    replace_one_class_svm_results(dataset_id, result_dataframe)
    ...
    print("One-Class SVM training completed successfully.")
