...
def load_labeled_feature_measurements(dataset_id: int) -> pd.DataFrame:
    ...
    if len(np.unique(dataframe["original_label"])) < 2:
        raise RuntimeError("Gradient Boosting requires both normal and anomaly examples.")
...
def train_gradient_boosting(dataframe: pd.DataFrame, threshold: float) -> pd.DataFrame:
...
def replace_gradient_boosting_results(dataset_id: int, results: pd.DataFrame) -> None:
    execute_query(
        """
        DELETE FROM anomaly_results
        WHERE dataset_id = %s
          AND model_name = 'Gradient Boosting';
        """,
        (dataset_id,),
    )
...
def train_gradient_boosting_for_active_dataset() -> int:
    dataset_id = get_active_dataset_id()
    threshold = get_threshold()

    dataframe = load_labeled_feature_measurements(dataset_id)
    results = train_gradient_boosting(dataframe, threshold)
    replace_gradient_boosting_results(dataset_id, results)

    print("Gradient Boosting results written to anomaly_results.")
    return dataset_id


def main():
    train_gradient_boosting_for_active_dataset()
