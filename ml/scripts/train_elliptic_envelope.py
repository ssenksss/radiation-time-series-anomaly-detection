...
def train_elliptic_envelope(dataframe: pd.DataFrame, threshold: float) -> pd.DataFrame:
    ...
    print(f"Threshold used for Elliptic Envelope sensitivity: {threshold}")
    print(f"Calculated contamination: {contamination}")

    return model_dataframe


def replace_elliptic_envelope_results(dataset_id: int, results: pd.DataFrame) -> None:
    execute_query(
        """
        DELETE FROM anomaly_results
        WHERE dataset_id = %s
          AND model_name = 'Elliptic Envelope';
        """,
        (dataset_id,),
    )
...
def train_elliptic_envelope_for_active_dataset() -> int:
    dataset_id = get_active_dataset_id()
    threshold = get_threshold()

    feature_dataframe = load_feature_measurements(dataset_id)
    result_dataframe = train_elliptic_envelope(feature_dataframe, threshold)
    replace_elliptic_envelope_results(dataset_id, result_dataframe)
    ...
    print("Elliptic Envelope training completed successfully.")
