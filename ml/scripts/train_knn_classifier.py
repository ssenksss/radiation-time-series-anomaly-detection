...
def load_labeled_feature_measurements(dataset_id: int) -> pd.DataFrame:
    ...
    if len(np.unique(dataframe["original_label"])) < 2:
        raise RuntimeError("KNN Classifier requires both normal and anomaly examples.")
...
def train_knn_classifier(dataframe: pd.DataFrame, threshold: float) -> pd.DataFrame:
...
def replace_knn_classifier_results(dataset_id: int, results: pd.DataFrame) -> None:
    execute_query(
        """
        DELETE FROM anomaly_results
        WHERE dataset_id = %s
          AND model_name = 'KNN Classifier';
        """,
        (dataset_id,),
    )
...
def train_knn_classifier_for_active_dataset() -> int:
    dataset_id = get_active_dataset_id()
    threshold = get_threshold()

    dataframe = load_labeled_feature_measurements(dataset_id)
    results = train_knn_classifier(dataframe, threshold)
    replace_knn_classifier_results(dataset_id, results)

    print("KNN Classifier results written to anomaly_results.")
    return dataset_id


def main():
    train_knn_classifier_for_active_dataset()
