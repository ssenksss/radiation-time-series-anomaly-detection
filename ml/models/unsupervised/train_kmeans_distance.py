...
MODEL_NAME = "K-Means Distance"


def train_kmeans_distance(dataframe: pd.DataFrame, threshold: float) -> Tuple[pd.DataFrame, Dict[str, float]]:
    ...
    n_clusters = min(5, max(2, len(train_dataframe) // 1000))
...
def train_kmeans_distance_for_active_dataset() -> Dict[str, float]:
    dataset_id = get_active_dataset_id()
    threshold = get_threshold()

    feature_dataframe = load_feature_measurements(dataset_id)
    result_dataframe, timing = train_kmeans_distance(feature_dataframe, threshold)
