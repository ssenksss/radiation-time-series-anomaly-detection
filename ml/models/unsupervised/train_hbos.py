...
this model builds histograms for each feature and checks how rare each value is
records with rare feature values receive higher anomaly scores
"""
...
def train_hbos(dataframe: pd.DataFrame, threshold: float) -> Tuple[pd.DataFrame, Dict[str, float]]:
    ...
    # hbos scores records by checking rare histogram bins
    model = HBOS(
    ...
    # normalize scores for easier comparison in reports
    ...
    print(f"Threshold used for HBOS severity: {threshold}")
...
def train_hbos_for_active_dataset() -> Dict[str, float]:
    dataset_id = get_active_dataset_id()
    threshold = get_threshold()

    feature_dataframe = load_feature_measurements(dataset_id)
    result_dataframe, timing = train_hbos(feature_dataframe, threshold)
