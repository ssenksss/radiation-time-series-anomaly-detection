...
def train_hbos(
    ...
    print(
        f"HBOS train IQR: {iqr}"
    )

    print(
        f"HBOS IQR multiplier: {IQR_MULTIPLIER}"
    )

    print(
        f"HBOS anomaly score cutoff: {anomaly_cutoff}"
    )
...
def train_hbos_for_active_dataset() -> Dict[str, float]:
    ...
    result_dataframe, timing = train_hbos(
        feature_dataframe,
    )
